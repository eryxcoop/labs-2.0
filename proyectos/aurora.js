...
glftLoader.load("aurora_low_stickers.glb", (glft) => {
  ...
  modelo3d.position.x = 4;
  modelo3d.position.y = -400;
  modelo3d.scale.z -= 0.6;
  modelo3d.scale.y -= 0.6;
  modelo3d.scale.x -= 0.6;
})
...
function moveSpaceWithMouse(e) {
  ...
  if (modelo3d) {
    modelo3d.rotation.x = e.clientX * -0.2;
    modelo3d.rotation.y = e.clientY * -0.001;
  }
...
function animate() {
  ...
  if (modelo3d) {
    modelo3d.position.y = Math.cos( time ) * 0.2 - 2;
    modelo3d.rotation.x = 0.2;
    modelo3d.rotation.y = time * -0.2;
  }
